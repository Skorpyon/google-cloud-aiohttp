...
from aiogoogle.cloud.streaming.buffered_stream import BufferedStream
from aiogoogle.cloud.streaming.exceptions import CommunicationError
from aiogoogle.cloud.streaming.exceptions import HttpError
from aiogoogle.cloud.streaming.exceptions import TransferInvalidError
from aiogoogle.cloud.streaming.exceptions import TransferRetryError
...
from aiogoogle.cloud.streaming.http_wrapper import RESUME_INCOMPLETE
from aiogoogle.cloud.streaming.stream_slice import StreamSlice
from aiogoogle.cloud.streaming.util import acceptable_mime_type
...
class _Transfer(SyncTransfer):
    ...
    def _initialize(self, http, url):
        ...
        if self.http is None:
            if not http:
                raise ValueError('Http connector not passed.')
            self._http = http
        self._url = url

    def __del__(self):
        if self._close_stream:
            loop = self._http.loop
            loop.create_task(self._stream.close())
...
class Download(_Transfer):
    ...
    async def initialize_download(self, http_request, http):
    ...
    async def _get_chunk(self, start, end, headers=None):
